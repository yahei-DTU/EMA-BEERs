...
!!! example "Exercise"
    ...
    3. Create a new page `docs/source/abv.md` and add it to the `nav` in `mkdocs.yaml`. Confirm it appears in the sidebar.

        The page should include a plot of the data generated by `generate_data` with a linear regression fit. Generate the plot in Python and save it as an image, then embed it in the Markdown:
        ...
        m, b = np.polyfit(df["abv_true"], df["abv"], 1)
        x = np.linspace(df["abv_true"].min(), df["abv_true"].max(), 100)

        fig, ax = plt.subplots()
        ax.scatter(df["abv_true"], df["abv"], alpha=0.4, s=10, label="Samples")
        ax.plot(x, m * x + b, color="red", label=f"Fit: y = {m:.2f}x + {b:.2f}")
        ax.set_xlabel("True ABV (%)")
        ax.set_ylabel("Measured ABV (%)")
        ax.legend()
        fig.savefig("docs/source/abv_plot.png", dpi=150, bbox_inches="tight")
        ```

        Then in `abv.md`:

        ```markdown
        ![ABV regression plot](abv_plot.png)
        ```
